...
La Figura~\ref{FIG:SHUFFLED_CORR} muestra que la estructura ordenada se mantiene aunque cambie el orden inicial de los activos, lo que valida que el resultado no es un artefacto visual. Esto significa que el patrón jerárquico proviene de las relaciones de similitud entre activos y no de su posición en la matriz. En términos metodológicos, la ordenación clásica es invariante al reetiquetado de entrada.
...
La Tabla~\ref{TB:COMPARACIONPAPER_T2} permite observar que el mensaje principal del paper se mantiene (QHRP competitivo y en primera posición), aunque cambie el ranking intermedio. Esto indica que la metodología es reproducible en lo esencial, pero no garantiza coincidencia exacta en todos los métodos cuando cambian datos y decisiones de preprocesado. Los métodos con rendimiento cercano entre sí son especialmente sensibles a estas variaciones.
...
La Figura~\ref{FIG:COMP_PAPER_FIG3_PAPER} fija el patrón de referencia que debe reproducir la metodología: aparición de bloques cuasi-diagonales tras ordenar. Su valor está en definir el criterio visual de validación estructural, no en demostrar por sí sola mejor rendimiento financiero.
...
La Figura~\ref{FIG:COMP_PAPER_FIG4_PAPER} muestra que la distancia cuántica no es solo una transformación matemática, sino una geometría útil para ordenar activos jerárquicamente. El enfoque cuántico produce información estructural aprovechable por HRP.
...
\begin{itemize}
	...
	\item \textbf{Cuántico:} la dimensión de estado crece exponencialmente como \(2^P\), por lo que el coste de simulación se convierte en el factor crítico al aumentar \(P\).
\end{itemize}
...
En el análisis del HRP clásico, el grupo más grande contiene 147 activos (de 169 totales), con las siguientes características:

\begin{table}[HRP Clásico — Mega-grupo]{TB:HRP_CLASSICAL_MAIN}{Estadísticas del grupo principal en HRP Clásico (\(N=147\)).}
\begin{tabular}{lc}
\hline
\textbf{Métrica} & \textbf{Valor} \\
\hline
Tamaño & 147 activos \\
Sectores distintos & 23 \\
Purity & 0.129 \\
Entropy & 0.866 \\
Sector dominante & Consumer Cyclical (12.9\%) \\
\hline
\end{tabular}
\end{table}

Este resultado revela un problema fundamental de la ordenación clásica: agrupa un ``mega-grupo'' heterogéneo que contiene simultáneamente:
...
En contraste, el grupo de criptomonedas (9 activos: BTC, ETH, SOL, BNB, AVAX, XRP, DOGE, ADA, DOT) presenta estadísticas ideales:

\begin{table}[HRP Clásico — Grupo Crypto]{TB:HRP_CLASSICAL_CRYPTO}{Estadísticas del grupo crypto en HRP Clásico.}
\begin{tabular}{lc}
\hline
\textbf{Métrica} & \textbf{Valor} \\
\hline
Tamaño & 9 activos \\
Sectores distintos & 1 \\
Purity & 1.000 \\
Entropy & 0.000 \\
Sector dominante & Crypto (100\%) \\
\hline
\end{tabular}
\end{table}

Este grupo homogéneo indica que los métodos jerárquicos sí pueden captar agrupamientos significativos cuando existe un factor de riesgo uniforme y dominante. El grupo crypto actúa como validación de que una Purity~=~1.0 es alcanzable y económicamente interpretable.

\subsubsection{QHRP Cuántico: Grupo de Diversificadores/Coberturas}
\label{subsubsec:qhrp_diversificadores}

En el análisis del QHRP, uno de los grupos principales (31 activos) corresponde a una familia de diversificadores y coberturas:

\begin{table}[QHRP — Grupo de Diversificadores]{TB:QHRP_DIVERSIFIERS}{Estadísticas del grupo de Diversificadores en QHRP (\(N=31\)).}
\begin{tabular}{lc}
\hline
\textbf{Métrica} & \textbf{Valor} \\
\hline
Tamaño & 31 activos \\
Sectores distintos & 12 \\
Purity & 0.290 \\
Entropy & 0.845 \\
Sector dominante & Fixed Income (29\%) \\
\hline
\end{tabular}
\end{table}

A diferencia del mega-grupo del HRP clásico, este grupo está explícitamente compuesto por activos que tienen funciones de cobertura complementarias:
...
Otro grupo principal de QHRP (25 activos) agrupa la renta variable sistemática:

\begin{table}[QHRP — Grupo de Renta Variable]{TB:QHRP_EQUITIES}{Estadísticas del grupo de Renta Variable en QHRP (\(N=25\)).}
\begin{tabular}{lc}
\hline
\textbf{Métrica} & \textbf{Valor} \\
\hline
Tamaño & 25 activos \\
Sectores distintos & Múltiples \\
Pureza (\textit{Purity}) & 0.400 \\
Entropía (\textit{Entropy}) & 0.825 \\
Sector dominante & ETFs de renta variable diversificada (40\%) \\
\hline
\end{tabular}
\end{table}
...
La tabla comparativa resume las diferencias conceptuales:
